...
const ProjectDetail: React.FC = () => {
  ...
  if (!project)
    return (
      <div className="min-h-screen flex items-center justify-center font-mono text-pcb-primary-light animate-pulse">
        LOADING_DATA_STREAM...
      </div>
    );

  const chartData = project.downloads.map((val, idx) => ({
    name: `Day ${idx + 1}`,
    downloads: val,
  }));
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-[1400px]">
      <Link
        href="/"
        className="inline-flex items-center gap-2 text-zinc-500 hover:text-pcb-primary-light mb-6 transition-colors font-mono text-xs uppercase tracking-wider group"
      >
      ...
      <div className="w-full bg-yellow-500/10 border border-yellow-500/20 p-2 mb-4 text-center">
        <p className="text-yellow-500 text-xs font-mono font-bold uppercase tracking-widest">
          ⚠ SIMULATION_MODE // THIS_PROJECT_IS_A_PROP ⚠
        </p>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6 h-[85vh]">
          ...
          <div className="bg-pcb-panel border border-pcb-border p-5 flex flex-col gap-4">
            <div className="aspect-video w-full bg-black border border-zinc-800 overflow-hidden relative group">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-2">
                <div className="font-mono text-[10px] text-pcb-primary-light">
                  REV. 1.0.4
                </div>
              </div>
            </div>
          ...
          <div className="bg-pcb-panel border border-pcb-border p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {project.tags.map((t) => (
                <span
                  key={t}
                  className="px-2 py-1 bg-zinc-900 border border-zinc-800 text-zinc-400 text-[10px] font-mono uppercase"
                >
                  {t}
                </span>
              ))}
                ...
                <div className="grid grid-cols-2 gap-4 my-8">
                  {project.specs.map((spec) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-pcb-primary-light font-mono">
                        {spec.value}
                      </div>
                    </div>
                  ))}
            ...
            {activeTab === "bom" && (
              <div className="font-mono text-xs">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button className="text-pcb-primary-light border border-pcb-primary/30 px-3 py-1 hover:bg-pcb-primary/10">
                    EXPORT.CSV
                  </button>
                </div>
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-zinc-900">
                    {project.bom.map((item, idx) => (
                      <tr
                        key={idx}
                        className="text-zinc-400 hover:bg-zinc-900/50 hover:text-white transition-colors"
                      >
                        <td className="py-3 pr-4 text-pcb-primary-light">
                          {item.partNumber}
                        </td>
                        <td className="py-3 pr-4">{item.manufacturer}</td>
                        <td className="py-3 pr-4 text-zinc-500 truncate max-w-[200px]">
                          {item.description}
                        </td>
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col bg-black border border-pcb-border h-full">
          ...
          <div
            className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar font-mono text-xs"
            ref={scrollRef}
          >
            {messages.map((msg, idx) => (
              ...
              >
                <span className="text-[10px] text-zinc-700 uppercase">
                  {msg.role === "user" ? "USR_INPUT" : "SYS_OUTPUT"}
                </span>
                <div
            ...
            {isThinking && (
              <div className="flex flex-col items-start gap-1">
                ...
                <div className="text-pcb-primary-light animate-pulse">
                  {">"} PROCESSING_REQUEST...
                </div>
              </div>
            )}
